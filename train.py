...
import numpy as np
import imageio
from preprocessing import access_font_data, FontDataManager
...
def conv2d_block(x, shape, strides, padding, scope='conv2d'):
    ...
    with tf.name_scope(scope):
        if not strides:
            strides = [1, 1, 1, 1]
        out_filters = shape[-1]
        W = tf.Variable(tf.random.truncated_normal(shape, stddev=0.01),
                        name="W")
...
def main(_):
    ...
    with tf.name_scope("input"):
        x = tf.compat.v1.placeholder(tf.float32, [None, 160, 160], name='x')
        y = tf.compat.v1.placeholder(tf.float32, [None, 80, 80], name='y')
    ...
    with tf.name_scope("train"):
        ...
        train_step = tf.compat.v1.train.RMSPropOptimizer(learning_rate).minimize(combined_loss)

    with tf.name_scope("convert_bitmaps"):
        convert_bitmap = tf.reshape(y_hat_image, shape=[-1, 80, 80])

    tf.compat.v1.summary.scalar('pixel_abs_loss', pixel_abs_loss)
    ...
    if FLAGS.mode == 'train':
        ...
        font_bitmaps = access_font_data(FLAGS.source_font, True)
